...
export default async function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: post }, { data: verifications }] = await Promise.all([
    supabase
      .from('posts')
      .select('*, users(name, role_level, company, job_role)')
      .eq('id', id)
      .single(),
    supabase
      .from('verifications')
  ...
  const appropriateCount = verifications?.filter(v => v.judgment === 'appropriate').length ?? 0
  const inappropriateCount = verifications?.filter(v => v.judgment === 'inappropriate').length ?? 0
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="flex items-center gap-2 pt-4 border-t border-gray-100">
            <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-sm font-bold text-blue-700">
              {(post.users as { name: string } | null)?.name?.[0] ?? '?'}
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-sm font-medium text-gray-800">
                  {(post.users as { name: string } | null)?.name ?? '알 수 없음'}
                </span>
                <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${ROLE_COLOR[(post.users as { role_level: string } | null)?.role_level ?? 'observer']}`}>
                  {ROLE_LABEL[(post.users as { role_level: string } | null)?.role_level ?? 'observer']}
                </span>
              </div>
              {(post.users as { company?: string; job_role?: string } | null)?.company && (
                <p className="text-xs text-gray-400">
                  {(post.users as { company: string; job_role: string }).company} · {(post.users as { company: string; job_role: string }).job_role}
                </p>
              )}
            </div>
          </div>
            ...
            <>
                ...
                <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden flex">
                  {appropriateCount > 0 && (
                    <div
                      ...
                      style={{ width: `${(appropriateCount / totalCount) * 100}%` }}
                    />
                  )}
                  {inappropriateCount > 0 && (
                    <div
                      className="h-full bg-red-400 rounded-r-full transition-all"
                      style={{ width: `${(inappropriateCount / totalCount) * 100}%` }}
                    />
                  )}
                </div>
              </div>
              ...
              {verifications?.filter(v => v.comment).map((v, i) => (
                <div key={i} className="flex gap-3 py-3 border-t border-gray-50 first:border-0">
                  <span className={`text-lg mt-0.5 ${v.judgment === 'appropriate' ? 'text-green-500' : 'text-red-500'}`}>
                    {v.judgment === 'appropriate' ? '✅' : '❌'}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="text-sm font-medium text-gray-700">
                        {(v.users as { name: string } | null)?.name ?? '검증자'}
                      </span>
                      <span className={`text-xs px-1 py-0.5 rounded ${ROLE_COLOR[(v.users as { role_level: string } | null)?.role_level ?? 'observer']}`}>
                        {ROLE_LABEL[(v.users as { role_level: string } | null)?.role_level ?? 'observer']}
                      </span>
                    </div>
